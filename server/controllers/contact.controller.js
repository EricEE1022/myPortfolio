...
const remove = async (req, res) => {
  try {
    ...
    //deletedContact = undefined
    res.json({ deletedContact, message: "Successfully deleted!" });
  } catch (err) {
    return res.status(400).json({
      error: errorHandler.getErrorMessage(err),
    });
...
const removeAll = async (req, res) => {
  try {
    let deletedAllContact = await Contact.deleteMany({});
    res.json({
      message: `${deletedAllContact.deletedCount} All contacts deleted successfully.`,
    });
  } catch (err) {
    return res.status(400).json({
      error: errorHandler.getErrorMessage(err),
